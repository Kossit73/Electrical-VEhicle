...
def _format_currency(value: float) -> str:
    return f"$ {value:,.0f}"
config_tab, analysis_tab = st.tabs(["Configuration", "Analysis"])
...
with config_tab:
    ...
    if vehicle_key == "Custom":
        ...
        with vehicle_cols[0]:
            ...
            purchase_price = st.number_input(
                "Purchase price", min_value=10_000.0, value=45_000.0, step=1_000.0
            )
            battery_capacity = st.number_input(
                "Battery capacity (kWh)", min_value=10.0, value=60.0, step=1.0
            )
            epa_range = st.number_input(
                "EPA range (miles)", min_value=100.0, value=250.0, step=10.0
            )
            efficiency = st.number_input(
                "Efficiency (kWh/mile)", min_value=0.1, max_value=0.6, value=0.24, step=0.01
            )
        with vehicle_cols[1]:
            warranty_years = st.slider(
                "Battery warranty (years)", min_value=3, max_value=12, value=8
            )
    ...
    if financing_key == "Custom":
        financing_cols = st.columns(2)
        with financing_cols[0]:
            loan_amount = st.number_input(
                "Loan amount",
                min_value=0.0,
                value=max(vehicle.purchase_price * 0.8, 0.0),
                step=1_000.0,
            )
            loan_term = st.slider(
                "Loan term (years)", min_value=1, max_value=10, value=5
            )
        with financing_cols[1]:
            down_payment = st.number_input(
                "Down payment",
                min_value=0.0,
                value=vehicle.purchase_price - loan_amount,
                step=1_000.0,
            )
    ...
    if energy_key == "Custom":
        ...
        with energy_cols[1]:
            home_share = st.slider(
                "Home charging share", min_value=0.0, max_value=1.0, value=0.7, step=0.05
            )
            dc_share = st.slider(
    ...
    if incentive_key == "None":
        ...
        incentive_cols = st.columns(2)
        with incentive_cols[0]:
            federal_credit = st.number_input(
                "Federal tax credit", min_value=0.0, value=7_500.0, step=500.0
            )
            local_rebate = st.number_input(
                "Local rebate", min_value=0.0, value=0.0, step=250.0
            )
        with incentive_cols[1]:
            state_credit = st.number_input(
                "State credit", min_value=0.0, value=2_000.0, step=250.0
            )
            utility_rebate = st.number_input(
                "Utility rebate", min_value=0.0, value=500.0, step=250.0
            )
    ...
    st.success("Configuration saved. Switch to the Analysis tab to review results.")
...
with analysis_tab:
    ...
    summary_cols[0].metric(
        "Net ownership cost", _format_currency(summary["net_ownership_cost"])
    )
    ...
    if show_comparison:
        ...
        traditional_options: Dict[str, Optional[Dict[str, float]]] = {
            "None": None,
            **SAMPLES["traditional"],
        }
        hybrid_options: Dict[str, Optional[Dict[str, float]]] = {
            "None": None,
            **SAMPLES["hybrid"],
        }
            ...
            comparison_rows.append(
                {
                    ...
                    "Cost per mile": hybrid_summary["total_annual_cost"]
                    / energy_params.annual_miles_driven,
    ...
    if show_sensitivity:
        ...
        variation = st.slider(
            "Variation ±%", min_value=5, max_value=50, value=20, step=5
        )
        ...
        try:
            ...
            st.dataframe(
                scenarios_df[
                    ["change_percent", "new_value", "cost_per_mile", "impact_percent"]
                ]
                .rename(
                    columns={
                        "change_percent": "Δ %",
    ...
    st.success("Analysis complete. Revisit the Configuration tab to adjust inputs.")
